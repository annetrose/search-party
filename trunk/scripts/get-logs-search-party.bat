@rem Download the logs to a local file for search-party.

@rem Run this from the project directory like this:
@rem
@rem cd s:\d\sp\search-party\
@rem scripts\get-logs-search-party.bat

c:\p\Python27\python27.exe c:\p\AppEngine\appcfg.py --email=alexanderjquinn@gmail.com --num_days 0 --include_vhost --severity=0 --application=search-party --version=1 request_logs .\ "logs\search-party.log" 

@rem See:
@rem http://blog.dantup.com/2009/12/downloadingexporting-app-engine-logs.html

@rem Alteratively, you could go to:
@rem https://appengine.google.com/logs?app_id=s~search-party#offset-Cg5O8el0AAfkeI34AcwAAA==
