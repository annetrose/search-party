...
#        if updates[0]['type'] == "query":
#            send_error(from_person, "Query not sent. Teacher not logged in.")
#            log( "****** ERROR: QUERY NOT SENT. No current client IDs for {0}.".format(to_person))
...
def send_update_query(student, teacher, task_idx, query):
...
def send_update_link_followed(student, teacher, task_idx, query, url, title):
...
def send_update_link_rated(student, teacher, task_idx, url, is_helpful):
